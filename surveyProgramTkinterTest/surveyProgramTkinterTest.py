...
class Survey:
    ...
    def ask_questions(self, root):                            #asks questions
        answers = []
        text = tk.Text(root)
        for index, row in self.questions.iterrows():    #goes through asking each question
            while True:
                ...
                text.pack()     
                root.mainloop()                           #loops through asking same question until user says Y or N(temperary while console based)
...
def main():
    ...
    answers = survey.ask_questions(root)
